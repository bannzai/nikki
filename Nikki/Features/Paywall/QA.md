...
- [ ] **特典が3件表示される**: 特典として「複数端末で同期」「テーマを増やす (背景画像を含む)」「1秒きざみの自動ロック」の3件が説明つきで表示され、スクロール末尾に暗号化の約束のフッター文言が出る (issue #109 でテンプレートの無料枠 #94 を撤去したため「テンプレート無制限」の特典を削除。4件時代の確認は 2026-08-24 のエビデンス参照)
