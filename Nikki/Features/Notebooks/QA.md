...
## 5. テンプレート作成は常に無料 (issue #109)

テンプレート (ノート) の作成に件数の上限はない。既定 4 テンプレートのシード (PR #98) と無料枠 2 件 (issue #94) の矛盾を、無料枠の撤去で解消した (issue #109。ユーザー決定「テンプレートは全部無料」)。

- [ ] **件数によらず作成できる**: テンプレートが何件あっても (既定シード 4 件の初期状態を含む)、「＋ 新しいテンプレート」フッタは通常表示のままで、タップすると作成フォームへ進める。ロック表示・ペイウォールは出ない (設定の管理一覧・エディタのテンプレート一覧の両方)
  - 自動化: manual（フッタの表示と遷移の目視確認のため。判定関数 canCreateNotebook は issue #109 で撤去済みでロジックのテスト対象がない）
- [ ] **既定のテンプレートの復元は常に実行できる**: テンプレートが何件あっても「既定のテンプレートを復元」で不足分が末尾に追加され、ペイウォールは開かない (重複しない冪等な挙動はセクション 4 の既存項目のとおり)
  - 自動化: manual（復元結果の目視確認のため）
