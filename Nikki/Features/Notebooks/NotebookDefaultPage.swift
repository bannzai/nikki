...
/// 設定 > 既定のテンプレート の選択一覧。新規日記の書き出しに使うテンプレートを選ぶ。
/// macOS の confirmationDialog(NSAlert)はボタン4個までしか出せず、テンプレートは自由に増やせるため、
/// ダイアログではなく一覧画面で選ぶ。
struct NotebookDefaultPage: View {
    ...
    var body: some View {
        // 未設定のときは新規作成と同じ解決(先頭のテンプレート)を選択中として見せる。
        let defaultNotebook = notebooks.first { $0.id.uuidString == defaultNotebookID } ?? notebooks.first
        VStack(spacing: 0) {
            InkNavBar(leading: .back, center: .title(String(localized: "Default template")), onLeading: { dismiss() })
            ScrollView {
